...
<div class="bg-red-500 p-4">
    <strong class="text-black text-3xl pl-4 text-center">{{ $message }}</strong>
</div>
@endif
...
<body style="background-image:linear-gradient(rgba(212, 212, 212, 0.1),rgba(212,212,212,0.1)),
    ...
    <div class="pt-8 pb-4 px-8 grid justify-end">
        <a href="{{url()->previous()}}" class="p-3 border-none italic text-black bg-black text-lg">
            Go Back &rarr;
        </a>
    </div>
    <h1 class="text-center text-3xl text-black">CHOOSE YOUR PET</h1>
    <section class="flex flex-wrap justify-center gap-3 p-12 w-full">
        @foreach ($pets->chunk(1) as $serviceChunk)
        <div
            class="max-w-sm bg-black rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
            @foreach ($serviceChunk as $pets)
            <img src="{{ asset('uploads/pets/'.$pets->images)}}" alt="I am A Pic" width="400"
                style="max-height: 12rem;">
            <div class="p-3">
                <h5 class="mb-2 text-2xl font-bold tracking-tight">{{ $pets->pets_name }}
                </h5>
                <p class="mb-2 text-lg font-bold">{{ $pets->type }}</p>
                <div class="grid grid-flow-col gap-2">
                    <a href=" {{ route('transaction.addpets', ['id'=>$pets->id]) }} " class="btn btn-primary"
                        role="button"><i class="fas fa-cart-plus"></i> Add pets</a>
    ...
    <h1 class="text-center text-3xl text-black pt-3">CHOOSE YOUR SERVICE</h1>
